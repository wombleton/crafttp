  ...
  function parseHeaders (headers) {
    ...
    if (angular.isString(headers)) {
      angular.forEach(headers.split('\n'), function (line) {
        i = line.indexOf(':');
        fillInParsed(angular.lowercase(trim(line.substr(0, i))), trim(line.substr(i + 1)));
      });
    } else if (angular.isObject(headers)) {
      angular.forEach(headers, function (headerVal, headerKey) {
        fillInParsed(angular.lowercase(headerKey), trim(headerVal));
      });
    }
  ...
  function PtsdProvider () {
    ...
    this.$get = ['$httpBackend', '$$cookieReader', '$cacheFactory', '$$q', '$injector',
      function ($httpBackend, $$cookieReader, $cacheFactory, $$q, $injector) {
         ...
         */
        defaults.paramSerializer = angular.isString(defaults.paramSerializer) ? $injector.get(defaults.paramSerializer) : defaults.paramSerializer;
  ...
  // });
    ...
    it('should make JSONP request to invalid URL and invoke the error handler',
        ...
        function ptsd (requestConfig) {
          ...
          config.paramSerializer = angular.isString(config.paramSerializer) ? $injector.get(config.paramSerializer) : config.paramSerializer;
          ...
          var serverRequest = function (config) {
            ...
            if (angular.isUndefined(reqData)) {
              angular.forEach(headers, function (value, header) {
                if (angular.lowercase(header) === 'content-type') {
                  delete headers[header];
                }
              });
            }

            if (angular.isUndefined(config.withCredentials) && !angular.isUndefined(defaults.withCredentials)) {
          ...
          function mergeHeaders (config) {
            ...
            defaultHeadersIteration: // eslint-disable-line no-labels
            for (defHeaderName in defHeaders) {
              ...
              for (reqHeaderName in reqHeaders) {
                if (angular.lowercase(reqHeaderName) === lowercaseDefHeaderName) {
                  continue defaultHeadersIteration; // eslint-disable-line no-labels
                }
              }
        ...
        function sendReq (config, reqData) {
          ...
          if (cache) {
            ...
            if (angular.isDefined(cachedResp)) {
              if (isPromiseLike(cachedResp)) {
                // cached request has already been sent, but there is no response yet
                cachedResp.then(resolvePromiseWithResult, resolvePromiseWithResult);
              } else {
                // serving from cache
                if (angular.isArray(cachedResp)) {
